 ...
 * If the store ever meters something for real - a subscription's included
 * deliveries, say - that belongs on the customer's plan and goes through
 * `track`, not through here.
 *
 * In memory rather than in a database because it has to be resettable and
 * disposable. A demo that remembers yesterday's visitor's empty shelves is a
 * demo that shows a broken shop.
 *
 * On a serverless host - which is what this deploys to - that lifetime is one
 * isolate's, so two visitors can see different shelves and a cold start
 * restores the defaults on its own. For a sample store that is a feature rather
 * than a bug, and it is why nothing here is presented as durable.
 */
 ...
 * fully booked is an answer the drawer renders, not a fault. Nothing is taken
 * on a refusal, so the same call is safe to make again after a reset.
 *

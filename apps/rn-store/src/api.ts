...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    throw new Error(
      `Could not reach the store API at ${url}. Is it running, and is that address reachable from this device? See src/config.ts.`,
      { cause },
    );
  }
 ...
 * `depotstore://`. The app names its scheme rather than the server assuming
 * one, because the same API serves more than one of these stores.
 */
export const startSubscription = (input: {
  ...
  request<SubscriptionResult>('/api/subscribe', {
    method: 'POST',
    body: JSON.stringify({ ...input, scheme: APP_SCHEME }),
  });

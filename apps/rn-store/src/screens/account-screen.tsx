...
import { HoneystickError, useCustomer } from '@honeystick/react-native';
...
/**
 * Where a subscriber manages what they are paying for, on bare React Native.
 *
 * The Expo store's account screen, all but line for line - the differences are
 * `Linking.openURL` in place of `WebBrowser`, and React Navigation in place of
 * expo-router. Nothing about the Honeystick half changes, which is the thing
...
export default function AccountScreen() {
   ...
   * asking them to.
   *
   * It also covers the gap this app has and the Expo one does not: there is no
   * `openAuthSessionAsync` here to tell us the browser closed, so without the
   * stream the first thing that would refresh this screen is the shopper
   * pulling it down.
   */
   ...
   *
   * The real browser, never a WebView this app controls. The shopper cannot see
  ...
  const onUpdateCard = useCallback(async () => {
    ...
    setFailure(null);
    try {
      await Linking.openURL(await updateCard());
      /**
       * Nothing is awaited past the handoff, unlike in the Expo store.
       * `WebBrowser.openAuthSessionAsync` resolves when the browser closes;
       * `Linking.openURL` resolves as soon as the other app has been asked to
       * open, which tells us nothing about what happened there. The refresh
       * therefore belongs to coming back - see the AppState listener in
       * App.tsx - rather than to this call.
       */
    } catch (cause) {
      setFailure(
  ...
  }, [cancel, forget, navigation, subscription]);

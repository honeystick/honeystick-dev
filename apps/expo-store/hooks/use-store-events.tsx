 ...
 * a browser would read a stream. This replaces a hand-rolled parser over
 * `expo/fetch` that worked but was ours to maintain, and it is a native client
 * (OkHttp on Android, NSURLSession on iOS) piped over JSI, so frames arrive
 * unbuffered.
 *
...
export function StoreEventsProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    /**
     * The library is a Nitro module, so there is no web build of it.
     *
     * The Expo web target would otherwise throw inside
     * `NitroModules.createHybridObject` on mount, taking the screen with it.
     * Reported as offline rather than silently skipped, because a stream that
     * was never going to connect should not read as one that is merely quiet.
     */
    if (Platform.OS === 'web') {
      setStatus('offline');
      return;
